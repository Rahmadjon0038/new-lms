...
import { useTeacherGuideLevelDetail } from '../../../../../../hooks/guides';

const MAIN_COLOR = '#A60E07';

const TeacherMainPdfPage = () => {
  const { courseId, levelId } = useParams();
  const router = useRouter();

  const { data, isLoading } = useTeacherGuideLevelDetail(levelId);
  const mainPdf = data?.main_pdf || null;
  const fileUrl = useMemo(
    () => mainPdf?.protected_file_url || `/api/teacher/guides/levels/${levelId}/main-pdf/file`,
    [mainPdf, levelId]
  );
...
export default TeacherMainPdfPage;
